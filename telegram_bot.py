...
class TelegramNotifier:
    ...
    def send_signal(self, target_chat_id, symbol, signal_data, exchange_name="BINANCE"):
        ...
        message = (
            ...
            f"Tín hiệu: {icon}\n\n"
            f"🎯 **Bước 1: RẢI VỐN BẮT ĐÁY (DCA)**\n"
            f"- Vùng Kê Lệnh Đẹp (Từ EMA 21 -> EMA 50): `{signal_data.get('zone_min', 'N/A')}` đến `{signal_data.get('zone_max', 'N/A')}`\n"
            f"- Mức Ký Quỹ (Gợi ý): `{signal_data.get('margin_desc', '100 USD')}`\n"
            f"*(Khuyên dùng: Chia số vốn trên làm 2-3 lệnh nhỏ rải đều trong vùng giá này)*\n"
            f"- Nhập tổng Quy mô Vị thế (Size 10x): `{signal_data.get('pos_usd', '1000')} USD`\n"
            f"- 🛡️ Nhập Stoploss Tuyệt Đối: `{signal_data['sl']}`\n"
            f"*(Lưu ý: Nếu giá sập đứt vùng EMA 50 và chạm mốc SL này anh em chỉ mất khoảng 10 USD)*\n\n"
